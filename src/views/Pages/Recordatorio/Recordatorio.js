...
export default function Recordatorio() {
    ...
    const ListarRecordatiorios = async () => {
        const { data, status } = await BmHttp().get('recordatorio/'+GetTokenDecoded().cuenta_id)
    ...
    <>
        <Container fluid>
            <div className="d-flex justify-content-start mb-3">
                <button className="button-bm active ml-2" onClick={()=>setShow(true)}>
                    CREAR RECORDATORIO
                </button>
            </div>
            ...
            <Modal.Header>
                ...
                <Modal.Body>
                    <div className="form-group">
                        ...
                        <input type="text" name="contacto_id" value={recordatorio.contacto_id} onChange={handleOnchange} className="form-control" />
                    </div>
                    <div className="form-group">
                        <label>Conexion</label>
                        <select className="form-control" id="bot_id" name="bot_id" value={recordatorio.bot_id} onChange={handleOnchange}>
                    ...
                    <div className="flex-row d-flex">
                        ...
                        <div className="form-group w-50 mx-1">
                            <label>Fecha</label>
                            <input type="date" name="fecha" value={recordatorio.fecha} onChange={handleOnchange} className="form-control" />
                        </div>
                        <div className="form-group w-50 mx-1">
                            <label>Hora</label>
                            <input type="time" name="hora" value={recordatorio.hora} onChange={handleOnchange} className="form-control" />
                        </div>

                    </div>
                    <div className="form-group">
                        <label>Nota</label>
                        <textarea name="nota" value={recordatorio.nota} onChange={handleOnchange} className="form-control"></textarea>
                    </div>
                    <div className="form-group">
                        <label>Tipo</label>
                        <select name="tipo" value={recordatorio.tipo} onChange={handleOnchange} className="form-control">
                    ...
                    <div className="form-group">
                        ...
                        name="color" value={recordatorio.color} onChange={handleOnchange} className="form-control" />
                    </div>
                </Modal.Body>
                <Modal.Footer>
                    <button className="button-bm" onClick={handClose}>CANCELAR</button>
                    <button className="button-bm active" onClick={CrearRecordatorio}>GUARDAR</button>
                </Modal.Footer>
            </Modal>
